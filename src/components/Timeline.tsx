...
export function Timeline({ items, title = 'Latest' }: TimelineProps) {
  return (
    <section className="h-[calc(100vh-350px)]">
      ...
      <div className="relative h-[calc(100%-48px)] overflow-y-auto
        ...
        {/* Container for timeline line and content with left padding */}
            ...
            {items.map((item, index) => (
              <div key={index} className="group relative">
                ...
                <div className="absolute -left-[26px] top-[14px]">
                  {item.type === 'project' ? (
                    <div className="relative z-10 bg-white dark:bg-[#1a1a1a] flex items-center justify-center w-[18px] h-[18px] transition-colors group-hover:text-blue-500 text-gray-700 dark:text-gray-400 dark:group-hover:text-blue-400">
                      <FolderGit2 size={16} className="transition-colors" />
                    </div>
                  ) : (
                    <div className="w-[9px] h-[9px] rounded-full bg-gray-400 dark:bg-gray-500 relative z-10 transition-colors group-hover:bg-blue-500 dark:group-hover:bg-blue-400 ml-1" />
                  )}
                </div>
                ...
                <Link
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <h3 className={`text-[15px] font-medium truncate transition-colors ${'group-hover:text-blue-600 dark:text-gray-200 dark:group-hover:text-blue-400'
                        }`}>
                        {item.title}
                      </h3>
                    </div>
